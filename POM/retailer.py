...
class Retailerpage:
    ...
    def placeorder(self, item_quantity,): # [(item,quantity),(item,quantity)]
        ...
        for item,quantity in item_quantity:
            ...
            item_price = self.wrapper.read_text(literal_eval(str(self.current_price).replace("{item}", item)))
            calc_price = self.wrapper.read_text(literal_eval(str(self.calculated_price).replace("{item}",item)))
    ...
    def check_placed_order(self, item_quantity):
        self.driver.get(r"http://49.249.28.218:8081/AppServer/Supply_Chain_Management/retailer/view_my_orders.php")
        self.wrapper.click_element(self.my_order_lnk)
        self.wrapper.click_element(self.recent_detail_lnk)
        checkout_order = []
        for item,_ in item_quantity:
            checkout_order.append(self.wrapper.read_text(literal_eval(str(self.quantity).replace("{item}",item))))
