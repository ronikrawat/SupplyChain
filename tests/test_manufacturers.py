...
class Test_Manufacturers:
    ...
    def test_change_password(self, pages, username, password, user_type, cpassword):
        ...
        actual = pages.manufacturer._change_password(password, cpassword, cpassword)
    ...
    def test_invoice_display(
        ...
        assert (
            "Invoices" == page_title
        ), f"page title is {page_title} instead of Invoices"
        pages.invoicepage.filter_invoice(invoice_option, invoice_id)
        page_title = pages.invoicepage.check_invoice_detail_page().strip()
        assert (
            "Sales Invoice" == page_title
        ), f"page title is {page_title} instead of Sales Invoice"
